...
function checkArtworkParent(e) {
  ...
  if (parent) {
    ...
    let target;
    try {
      target = e.app.findRecordById("artwork", parent);
    } catch (err) {
      // Without this the request dies on a bare 404, which reads as if it were
      // THIS artwork that had vanished - the exact opaque failure the guards
      // above exist to avoid. It happens for real: the whole work is deleted
      // while someone has the form of one of its elements open.
      throw new BadRequestError(
        "The whole work this artwork points at does not exist any more. Someone may have " +
          "deleted it: choose another one in `parent`, or attach this artwork to an exhibition.",
      );
    }
